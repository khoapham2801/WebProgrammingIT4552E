        ...
        if (!$connect) {
            ...
            if ($resource = mysqli_query($connect, $SQLcmd)){
                ...
                while ($row = mysqli_fetch_object($resource) ) {
                    $mobileList[] = new EntityMobile($row->id, $row->brandId, $row->name, $row->platform, $row->chip, $row->rear_camera, $row->front_camera, $row->memory, $row->price, $row->screen, $row->discount, $row->img);
                }
                $jsonString = json_encode($mobileList);
                return $jsonString;
            } else {
                die ($errorMessage);
            }
